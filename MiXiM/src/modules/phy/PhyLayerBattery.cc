...
Decider* PhyLayerBattery::getDeciderFromName(const std::string& name,
        ParameterMap& params) {
...
void PhyLayerBattery::handleHostState(const HostState& state) {
    if (state.get() != HostState::ACTIVE
            && radio->getCurrentState() != MiximRadio::SLEEP) {
        coreEV
...
simtime_t PhyLayerBattery::setRadioState(int rs) {
    ...
    if (battery) {
        if (battery && battery->getState() != HostState::ACTIVE
                && rs != MiximRadio::SLEEP && prevState != rs) {
