...
std::size_t WakeupBaseDecider::tProcessingSignal::interferenceWith(const first_type& frame) {
...
void WakeupBaseDecider::tProcessingSignal::startProcessing(first_type frame, second_type state) {
    first  = frame;
    second = state;
    if (first != NULL) {
...
simtime_t WakeupBaseDecider::processSignal(airframe_ptr_t frame) {
...
double WakeupBaseDecider::getFrameReceivingPower(airframe_ptr_t frame) const {
...
simtime_t WakeupBaseDecider::processNewSignal(airframe_ptr_t frame) {
...
DeciderResult* WakeupBaseDecider::createResult(const airframe_ptr_t frame) const
{
    return new DeciderResult(!frame->hasBitError());
}

simtime_t WakeupBaseDecider::processUnknownSignal(airframe_ptr_t frame)
{
	opp_error("Unknown state for the AirFrame with ID %d", frame->getId());
	return notAgain;
}

ChannelState WakeupBaseDecider::getChannelState() const {
...
simtime_t WakeupBaseDecider::handleChannelSenseRequest(ChannelSenseRequest* request)
{
...
simtime_t WakeupBaseDecider::handleNewSenseRequest(ChannelSenseRequest* request)
{
...
void WakeupBaseDecider::channelChanged(int newChannel) {
    currentSignal.clear();
...
void WakeupBaseDecider::handleSenseRequestEnd(CSRInfo& requestInfo) {
	//assert(canAnswerCSR(requestInfo) <= phy->getSimTime());
	answerCSR(requestInfo);
}

WakeupBaseDecider::eSignalState WakeupBaseDecider::getSignalState(const airframe_ptr_t frame) const {
	if (frame == currentSignal.first)
...
WakeupBaseDecider::eSignalState WakeupBaseDecider::setSignalState(const airframe_ptr_t frame, WakeupBaseDecider::eSignalState newState) {
    if (frame == currentSignal.first) {
...
simtime_t WakeupBaseDecider::getNextSignalHandleTime(const airframe_ptr_t frame) const {
    if (frame != currentSignal.first)
        return notAgain;
    switch(getSignalState(frame)) {
...
void WakeupBaseDecider::channelStateChanged()
{
...
simtime_t WakeupBaseDecider::canAnswerCSR(const CSRInfo& requestInfo) const
{
...
WakeupBaseDecider::channel_sense_rssi_t WakeupBaseDecider::calcChannelSenseRSSI(simtime_t_cref start, simtime_t_cref end) const {
...
void WakeupBaseDecider::answerCSR(CSRInfo& requestInfo)
{
...
Mapping* WakeupBaseDecider::calculateSnrMapping(const airframe_ptr_t frame) const
{
...
void WakeupBaseDecider::getChannelInfo( simtime_t_cref  start
                                , simtime_t_cref  end
...
WakeupBaseDecider::rssi_mapping_t
WakeupBaseDecider::calculateRSSIMapping( simtime_t_cref       start,
                                   simtime_t_cref       end,
